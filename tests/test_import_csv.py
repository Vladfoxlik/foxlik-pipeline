...
def selftest():
    ...
    # --- fallback по ссылке: pmp-строка узнается, а не плодит двойника -----
    # 🔴 tick при публикации через Postmypost пишет «Медиа ID» = pmp:<id> и
    # знает постоянную ссылку поста (дотягивается после выхода). Выгрузка
    # несет НАСТОЯЩИЙ media id Instagram - по id строки не совпадают, и
    # раньше импорт заводил двойника без механики: словарь получал ноль.
    pubs = FakeSheet([{"ID": "W36-01", "Дата": "2026-09-04",
                       "Площадка": "instagram",
                       "Ссылка": "https://www.instagram.com/reel/ABC123/",
                       "Медиа ID": "pmp:31879606", "Механика": "мама"}])
    mets = FakeSheet()
    выгрузка = HEAD + ('"18123456789","1","%s","FOXLIK","Ужин без мультиков",'
                       '"15","09/04/2026 18:07",'
                       '"https://www.instagram.com/reel/ABC123/",'
                       '"Видео Instagram","","За всё время",1000,900,10,3,1,0,2\n'
                       % OUR)
    stats = I.load(read(выгрузка), pubs, mets, report=True)
    assert stats["новых"] == 0, "строка с той же ссылкой - тот же ролик, а не новый: %r" % stats
    assert len(pubs.rows) == 1, "двойник без механики убил бы словарь"
    ...
    # Хронология выгорания без нее сортировалась по date.max, то есть никак.
    pubs2, mets2 = FakeSheet(), FakeSheet()
    новый = HEAD + ('"18999","1","%s","FOXLIK","Старый ролик","15",'
                    '"08/15/2026 12:00","https://www.instagram.com/reel/OLD9/",'
                    '"Видео Instagram","","За всё время",500,400,10,1,0,0,0\n'
                    % OUR)
    I.load(read(новый), pubs2, mets2)
    assert pubs2.rows[0]["Дата"] == "2026-08-15", "дата публикации из «Время публикации»: %r" % pubs2.rows[0]

    # 🔴 Аудит 02.09: в колонке «Вердикт» жили ДВА словаря слов - metrics
    # писал «залет/середина/провал», импорт - «прошел/не решено/провал».
    # Любой фильтр по вердикту молча терял бы половину строк. Канон один.
    pubs9, mets9 = FakeSheet(), FakeSheet()
    I.load(read(HEAD + line("HIT1", 1000, 2, 1)), pubs9, mets9)
    assert mets9.rows[0]["Вердикт"] == "залет", mets9.rows[0]["Вердикт"]
