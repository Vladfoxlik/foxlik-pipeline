...
def selftest():
    ...
    try:
        ...
        # 🔴 Аудит 02.09: id Postmypost - НЕ id медиа Instagram. Graph API
        # отвечает на него 400, строка не помечается замеренной и спамит
        # владельцу каждый день навсегда. Такие ролики ждут выгрузки CSV.
        чужой = pub("W36-01", "2026-09-01")
        чужой["Медиа ID"] = "pmp:31879606"
        m = build([чужой])
        assert m.due() == [], "id Postmypost нельзя слать в Graph API"

        # 🔴 холостая проба не замеряется: ее нет в эфире
        холостая = pub("W36-02", "2026-09-01")
        холостая["Соответствие"] = "холостой прогон"
        m = build([холостая])
        assert m.due() == [], "холостой прогон не выходил в эфир - мерить нечего"
    ...
    assert all(r["Что в кадре"] == "" and r["Хук"] == "" for r in added)

    # --- 🔴 страховка на случай, когда расписание не сработало ---
    # Замер 28.08: суточный cron «17 6 * * *» пропустил свой срок и не запустился
    # вовсе, а замер на Д7 - основа всей петли. Цепочка тактов живая, она и должна
    # подстраховать: если замер просрочен, запустить его самой.
    m = build([], [])
    assert m.overdue(), "отметки нет вовсе - замер обязан считаться просроченным"

    m.put_setting(M.KEY_LAST_RUN, TODAY.isoformat())
    assert not m.overdue(), "замер сделан сегодня, повторять его незачем"

    # вчерашний замер просроченным не считается: суточный ритм соблюден,
    # а лишний прогон - это лишняя сводка владельцу и лишние запросы к Meta
    m.put_setting(M.KEY_LAST_RUN, (TODAY - datetime.timedelta(days=1)).isoformat())
    assert not m.overdue(), "вчерашний замер еще в сроке"

    # 🔴 а вот пропуск больше суток - это и есть отказ расписания
    m.put_setting(M.KEY_LAST_RUN, (TODAY - datetime.timedelta(days=2)).isoformat())
    assert m.overdue(), "замера не было два дня, а страховка молчит"

    # мусор в отметке трактуется как «замера не было»: молча ждать нельзя
    m.put_setting(M.KEY_LAST_RUN, "позавчера")
    assert m.overdue(), "нечитаемая отметка должна означать «замера не было»"
    ...
    print("metrics selftest OK: счет гейта, продление токена и тревога, отбор на Д7, "
          ...
          "страховка ловит пропущенное расписание")
