...
def as_date(value):
    u"""Дата из таблицы или файла. Google отдает своим числом - его тоже понимаем."""
...
ШИРИНА = [55, 70, 110, 95, 130, 100, 400, 240, 210, 170, 240, 180]
...
def main(argv):
    ...
    if "--show" in argv:
        ...
        for строка in handout_rows(план, подсказки=подсказки_из_плана(план)):
            print(u"%s · выход %s · снять до %s · %s · %s"
                  % (строка[u"№"], строка[u"Дата выхода"], строка[u"Снять до"],
    ...
    if "--push" in argv:
        ...
        было = pull(лист)
        n = push(лист, handout_rows(план, подсказки=подсказки_из_плана(план)))
