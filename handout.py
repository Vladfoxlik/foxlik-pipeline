...
def первые_секунды(строка):
    u"""«Первые 3 секунды» для креатора: сначала чем открывается видео, потом смысл.

    🔴 Требование владельца 10.09: «не каждый ролик с ее лица». Первый кадр живет
    в своей колонке плана; без этой сборки он проверялся бы машиной, а креатор
    видел бы только фразу хука и снимал по-старому.
    """
    первый = (строка.get(u"Первый кадр") or u"").strip()
    хук = (строка.get(u"Хук") or u"").strip()
    if первый and хук:
        return u"%s. Смысл: %s" % (первый.rstrip(u"."), хук)
    return первый or хук
...
def handout_rows(план, сегодня=None, подсказки=None):
    ...
    for строка in план:
        ...
        out.append({
            ...
            u"Первые 3 секунды": первые_секунды(строка),
