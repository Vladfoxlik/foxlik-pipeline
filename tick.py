...
class Pipeline:
    ...
    def window_for(self, день):
        """Свободное окно дня: (номер, всего) или None, когда окна кончились.
        ...
        if len(занято) >= len(ОКНА_ДНЯ):
            return None
        return (len(занято), len(ОКНА_ДНЯ))
        ...
        for row in rows:
            ...
            if not self.caption_of(row):
                self._defer(row, "в ПЛАНЕ нет описания к посту для «%s»"
                            % (key or "?"))
                continue
            # 🔴 11.09: на день приходится столько роликов, сколько окон. Окна
            # кончились - строка ждет свободного дня, а не встает на ту же минуту,
            # что уже занятая (случай 10.09). Куда девать опоздавшие ролики
            # в полностью занятой неделе - решение владельца, вопрос В45.
    ...
    def publish(self, row):
        ...
        if self.pmp:
            try:
                ...
                slot, of = (self.window_for(_as_date(row.get(COL_DATE)) or self.today)
                            or (0, len(ОКНА_ДНЯ)))
