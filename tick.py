...
class Pipeline:
    ...
    def _taken(self, день):
        """Сколько роликов уже стоит в учете на этот день; None - учет не прочитался."""
        try:
            занято = {str(p.get("Медиа ID") or "").strip()
                      for p in self.pubs.read()
                      if _as_date(p.get("Дата")) == день}
        except Exception as e:
            self.say("окна дня не проверены: %s" % http.mask(str(e))[:120])
            return None
        занято.discard("")
        занято.discard(PENDING)
        return len(занято)

    def window_for(self, день):
        """Свободное плановое окно дня: (номер, всего) или None, когда окна кончились.

        🔴 Считается по УЧЕТУ, а не по плановому дню строки (замер 10.09).
        Два ролика встали на одну минуту: слоты брались из ПЛАНА по их плановым
        дням, а в эфир оба пошли в один день - опоздавший и сегодняшний. Учет
        знает, что на этот день уже стоит, чей бы план это ни был.

        🔴 Учет не прочитался - не повод держать эфир: берем первое окно и
        говорим вслух. Пропущенная публикация дороже возможного совпадения.
        """
        n = self._taken(день)
        if n is None:
            return (0, len(ОКНА_ДНЯ))
        if n >= len(ОКНА_ДНЯ):
            return None
        return (n, len(ОКНА_ДНЯ))

    def slot_for(self, день):
        """Место ролика в дне для расчета времени: плановое окно, а когда оба
    ...
    def _late_fits(self, rows, row):
        ...
        Всего окон в дне три: два плановых и 19:30 для опоздавших. Опоздавший
        не отнимает окно у СДАННОГО планового ролика этого дня (на приемке,
        одобрен или в публикации): такие ролики держат места заранее.
        Учет не прочитался - опоздавший ждет такт: сутки он уже ждал,
        а тройной выход в одно время хуже лишних пяти минут.
        """
        ...
        for r in rows:
            ...
            if день == self.today:
                резерв += 1
        # 🔴 А58, замер 17.09: оба плановых окна заняли опоздавшие W37-01 и W37-06,
        # плановый W37-10 окна уже не получит, а резерв за ним держал и 19:30 -
        # опоздавший W37-08 не выходил тоже. Держать можно только то, что осталось.
    ...
    def pick_due(self, rows):
        ...
        🔴 11.09: ПЛАН здесь не читается заранее. Раньше чтение стояло первой
        строкой, то есть на КАЖДОМ такте, даже без единой одобренной строки, и обрыв
        сети к Google становился тревогой владельцу про ролик, которого нет.
        План нужен только проверке подписи, а caption_of подгружает его сам.
        """
        for row in rows:
            ...
            if not self.caption_of(row):
                self._defer(row, "в ПЛАНЕ нет описания к посту для «%s»"
                            % (key or "?"))
                continue
            # 🔴 В45, 15.09: опоздавший (плановый день прошел) судится по СЕГОДНЯ.
            ...
            if self._is_late(row, when):
                if not self._late_fits(rows, row):
                    ...
                    continue
                self.sheet.set(row["_row"], COL_DATE, self.today.isoformat())
                row[COL_DATE] = self.today.isoformat()
                return row
    ...
    def resend(self, rows):
        ...
        if not ждут:
            return
        # 🔴 ПЛАН читается только когда есть что досылать (замер 11.09). Раньше
        # чтение стояло в начале, то есть на КАЖДОМ такте: обрыв сети к Google
        # превращался во владельцу «⚠️ Ролик уйдет в учет без механики» - про ролик,
        # которого не было. Подписи и оси нужны досылу, но только самому досылу.
        self._load_plan()
        ...
        try:
            ...
            if status_of(due) == PUBLISHED:
                ...
                try:
                    ...
                    pass
                return self.log
            if isinstance(e, postmypost.UploadError):
                # 🔴 Замер 13.09: W37-01 и W37-02 упали на заливке файла («The write
                # operation timed out») и получили терминальную ОШИБКУ. Кнопки по
                # такой строке не работают - ролики застряли, эфир 13.09 был пуст,
                # владелец получил сырой текст ошибки. Заливка идет ДО создания
                # публикации, поэтому возврат в очередь двойного эфира не дает.
                предел = 3                  # тактов подряд; внутри такта свои 3 попытки
                было = re.search(r"попытка (\d+) из", due.get(COL_REASON) or "")
                n = (int(было.group(1)) if было else 0) + 1
                название = due.get(COL_PLAN, "")
                if n < предел:
                    self.sheet.set_many(due["_row"], {
                        COL_STATUS: APPROVED,
                        COL_REASON: "обрыв связи при загрузке, попытка %d из %d" % (n, предел)})
                    self.say("строка %s: обрыв при загрузке файла, вернулась в очередь (%d из %d)"
                             % (due["_row"], n, предел))
                    self.bot.notify("⏳ Ролик «%s» не загрузился в сервис публикации: "
                                    "оборвалась связь. Вернул в очередь, повторю в следующем "
                                    "такте (попытка %d из %d). Делать ничего не нужно."
                                    % (название, n, предел))
                    return self.log
                self.sheet.set_many(due["_row"], {
                    COL_STATUS: FAILED,
                    COL_REASON: "обрыв связи при загрузке, %d такта подряд: %s"
                                % (n, http.mask(str(e))[:600])})
                self.say("строка %s -> ОШИБКА после %d обрывов загрузки" % (due["_row"], n))
                self.bot.notify("🔴 Ролик «%s» не опубликован: %d такта подряд не загрузился "
                                "в сервис публикации, связь обрывается. Нужна проверка - "
                                "напишите агенту." % (название, n))
                return self.log
            self.sheet.set_many(due["_row"], {
...
# Окно для опоздавших роликов (решение В45, 15.09): между двумя плановыми, внутри
# вечера - единственного окна, подтвержденного и нашим замером, и чужими данными
