...
def do_plan_replace(план, sheet, артикулы=None):
    ...
    без_артикула = plan_without_article(
        план, read_articles() if артикулы is None else артикулы)
    if без_артикула:
        raise SystemExit(
            "в подписи нет артикула WB своего товара: %s.\nРешение владельца 31.08 - "
            "подпись ведет на артикул. Номера лежат в data/артикулы.tsv."
            % ", ".join(без_артикула))
